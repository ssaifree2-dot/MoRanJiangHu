...
const 禁止当作物品名正则 = /旁白|西厢房|东厢房|卧房|厢房|宗门线|官府线|秘境线|江湖线|剧情|线索|名门之后|清晨|寒意|窗纸|木窗|棉被|床头|霜色/u;

const 清理物品名候选 = (value: string): string => (
    value
        .replace(/^[：:，,。.、\s]+|[：:，,。.、\s]+$/g, '')
        .replace(/^(本地|此地|当前地点|西厢房|东厢房|卧房|厢房)[·\-]/u, '')
        .trim()
);
...
export const 从剧情响应构建拍卖行投放参数列表 = (
    ...
    const initialPlotSeedIntent = context?.allowInitialPlotSeed === true && itemNames.length > 0;
    if (!auctionIntent && !rareIntent && !initialPlotSeedIntent) {
