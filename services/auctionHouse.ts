...
const 物品入市语义正则 = /拍卖行|牙行|黑市|寄售|流入市面|市面流通|有人出货|暗中兜售|悬赏流出|上架|入市|拍品|货品|摊客|卖家|出售|贩卖|叫价|标价|收购|挂出|摆出|挂售|兜售/u;
const 玩家私有物品语义正则 = /你|主角|少爷|自己|你的|自身|身边|枕边|怀中|手中|腰间|腰侧|背包|包袱|行囊|佩着|握着|拿着|穿着|装备|放在|收在|藏在|获得|拾得|捡到|缴获|搜出|得到|拿到|赠予|赏下|奖励|发现|取出|翻出/u;
...
const 清理物品名候选 = (value: string): string => {
    let name = value
        ...
        .replace(/^(那件|那柄|那把|这件|这柄|这把|一件|一柄|一把|一枚|一卷|一本|一块|一只|一个)/u, '')
        .trim();
    const 的Index = name.lastIndexOf('的');
    if (的Index >= 0 && 的Index < name.length - 1) {
        const core = name.slice(的Index + 1).trim();
        if (new RegExp(`^[\\u4e00-\\u9fa5A-Za-z0-9·]{1,8}${物品名后缀}$`, 'u').test(core)) {
            name = core;
        }
    }
    return name;
...
const 读取物品附近文本 = (text: string, itemName: string, radius = 36): string => {
    const normalized = 清理物品名候选(itemName);
    if (!normalized) return '';
    const directIndex = text.indexOf(normalized);
    if (directIndex >= 0) {
        return text.slice(Math.max(0, directIndex - radius), Math.min(text.length, directIndex + normalized.length + radius));
    }
    const fuzzyIndex = itemName && itemName !== normalized ? text.indexOf(itemName) : -1;
    if (fuzzyIndex >= 0) {
        return text.slice(Math.max(0, fuzzyIndex - radius), Math.min(text.length, fuzzyIndex + itemName.length + radius));
    }
    return '';
};

const 是否具体物品入市 = (text: string, itemName: string): boolean => {
    const context = 读取物品附近文本(text, itemName, 42);
    if (!context || !物品入市语义正则.test(context)) return false;
    const ownedContext = 读取物品附近文本(text, itemName, 24);
    if (玩家私有物品语义正则.test(ownedContext) && !/(寄售|上架|挂售|卖给|出售给|交给牙行|送入拍卖行)/u.test(ownedContext)) {
        return false;
    }
    return true;
};
...
export const 从剧情响应构建拍卖行投放参数列表 = (
    ...
    if (context?.useAIExtraction && context?.aiExtractionResult) {
        ...
        // 非开局种子必须有明确市场语义。稀有、获得、看见或持有物品都不能自动流入市场。
        if (!aiResult.是否有市场语义 && !context.allowInitialPlotSeed) {
            return { shouldDispatch: false, reason: 'AI 判断：本回合没有明确市场流通语义' };
        }
        
        // AI 提取的物品列表
        const aiItems = Array.isArray(aiResult.提取的物品列表)
            ? aiResult.提取的物品列表
                .map((item: any) => ({ ...item, 名称: 清理物品名候选(String(item?.名称 || '')) }))
                .filter((item: any) => item.是否合理 && 是否有效物品名(item.名称) && (context.allowInitialPlotSeed || 是否具体物品入市(responseText, item.名称)))
            : [];
        ...
        if (aiItems.length > 0) {
            ...
            const paramsList = aiItems.slice(0, context?.maxCount || 3).map((item: any) => {
                ...
                return {
                    ...
                    价格倍率: aiResult.是否有稀有物语义 ? 1.28 : 1.12,
            ...
            return {
                ...
                reason: aiResult.是否有市场语义 ? 'AI 判断：命中拍卖行/市场投放语义' 
                    : aiResult.是否有稀有物语义 ? 'AI 判断：命中带市场流通的稀有物语义'
                    : 'AI 判断：命中可交易物品入市语义',
                params: paramsList[0],
                paramsList
            };
        ...
        if (context.allowInitialPlotSeed) {
            ...
            return { shouldDispatch: false, reason: 'AI 判断：本回合没有具体物品入市线索' };
        }
    }
    ...
    // 必须有明确的市场语义，或者是开局剧情种子。稀有物出现/玩家获得不等于自动入市。
    if (!auctionIntent && !isInitialPlotSeed) {
        return { shouldDispatch: false, reason: '本回合没有明确市场流通语义' };
    }
    
    const itemNames = 提取明确物品名列表(text, context?.maxCount || (isInitialPlotSeed ? 3 : 1))
        .filter((itemName) => isInitialPlotSeed || 是否具体物品入市(text, itemName));
    ...
    if (itemNames.length === 0) {
        return { shouldDispatch: false, reason: '本回合没有具体物品入市线索' };
    }
        ...
        const priceBase = quality === '传说' ? 88000
            : quality === '绝世' ? 52000
                : quality === '极品' ? 24000
                    : quality === '上品' ? 8200
                        : 2600;
        const 来源描述 = 构建拍卖来源描述(itemName, auctionIntent, rareIntent);
        return {
            事件名称: eventName,
            来源描述,
            主线类型: mainline,
