...
export const 获取NSFW文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    if (!independent) return sharedNsfwConfig || 构建NSFW兜底配置(settings, sharedConfig);
    ...
    return 生图接口支持NSFW(result) ? result : (sharedNsfwConfig || 构建NSFW兜底配置(settings, sharedConfig));
};
