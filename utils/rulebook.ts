...
const 词条转条目 = (词条列表: unknown, context: string): 属性明细条目[] => {
    ...
    return 词条列表
        .map((entry: 物品词条) => {
            ...
            if (!attr || !Number.isFinite(value)) return null;
            return {
                标签: `${name} · ${attr}`,
    ...
    const groups: 物品明细分组[] = [
        {
            ...
            条目: [
                ...
                { 标签: '堆叠', 数值: `${格式数值(item?.堆叠数量, 1)} / ${格式数值(item?.最大堆叠, 1)}`, 依据: '堆叠数量决定出售、使用和总价值的数量基数。' },
        ...
        groups.push({
            ...
            条目: effects.length > 0
                ? effects.map((effect: any) => ({
                    标签: 读文本(effect?.目标属性, '目标属性'),
                    数值: `${读数(effect?.数值) > 0 ? '+' : ''}${格式数值(effect?.数值)}`,
                    依据: '消耗品.使用效果 会直接写回对应角色属性，并按最大值裁切。',
                }))
