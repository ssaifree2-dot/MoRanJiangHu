...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '8. 每回合都要复核主角与 NPC 档案：主角必须具备 `角色.技艺`，所有 NPC 必须具备天赋列表、出身背景、当前装备、背包、BUFF、DEBUFF、技艺；技艺至少覆盖炼器、炼丹、医术、阵法、符箓、机关、采集、鉴定等常见项。',
            '9. 技艺不是静态装饰：若正文或变量规划出现学习、试炼、炼制、采集、辨物、治伤、布阵、机关、符箓、丹器、找人学艺、读书学艺等事实，要按故事发展逻辑更新对应角色或 NPC 的技艺等级、熟练度与描述；初始技艺应由天赋列表、出身背景、身份职责和经历共同解释。',
